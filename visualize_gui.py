...
def plot_stripplot1(parent_frame, x, y, title=None):
    ...
    try:
        ...
        canvas = FigureCanvasTkAgg(fig, master=parent_frame)
        canvas.draw()
        canvas.get_tk_widget().pack(anchor="center", fill=tk.NONE, expand=False, padx=20, pady=20, ipadx=15, ipady=15)
    except Exception as ex:
        errorLabel.pack(side="top", padx=10, pady=10)
        errorLabel.config(text=f"Error: {ex}")
...
def plot_scatterplot1(parent_frame, x, y, title=None):
    ...
    try:
        ...
        canvas = FigureCanvasTkAgg(fig, master=parent_frame)
        canvas.draw()
        canvas.get_tk_widget().pack(anchor="center", fill=tk.BOTH, expand=True, padx=20, pady=20, ipadx=15, ipady=15)
    except Exception as ex:
        errorLabel.pack(side="top", padx=10, pady=10)
        errorLabel.config(text=f"Error: {ex}")
...
class App(tk.Tk):
    def __init__(self, title, size):
        ...
        self.visualization_frame = MyFrame(self, 900, 400, 'lightblue', 'solid', 1)
        self.visualization_frame.to_pack('left', True, tk.BOTH, 10, 10)
#Box Plot Widgets    
